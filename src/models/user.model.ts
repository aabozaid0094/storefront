...
export default class UserStore{
    ...
    async edit(user_id: Number, modified_user: User): Promise<User>{
        try {
            const connection = await client.connect()
            const query = 'UPDATE users SET user_email = $2, user_first_name = $3, user_last_name = $4, user_password_hash = $5, WHERE user_id = $1 returning *'
            const result = await connection.query(query, [user_id, modified_user.user_email, modified_user.user_first_name, modified_user.user_last_name, hashPassword(modified_user.user_password)])
            connection.release()
            return result.rows[0]
        } catch (error) {
