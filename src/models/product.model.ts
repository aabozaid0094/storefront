...
export default class ProductStore{
    ...
    async create(product: Product): Promise<Product>{
        try {
            ...
            const result = await connection.query(query, [product.product_name, product.product_price, product.product_category])
            connection.release()
    ...
    async edit(product_id: Number, modified_product: Product): Promise<Product>{
        try {
            const connection = await client.connect()
            const query = 'UPDATE products SET product_name = $2, product_price = $3, product_category = $4, WHERE product_id = $1 returning *'
            const result = await connection.query(query, [product_id, modified_product.product_name, modified_product.product_price, modified_product.product_category])
            connection.release()
            return result.rows[0]
        } catch (error) {
    ...
    async delete(product_id: Number): Promise<Product>{
        try {
            const connection = await client.connect()
            const query = 'DELETE FORM products WHERE product_id = $1 returning *'
            const result = await connection.query(query, [product_id])
            connection.release()
            return result.rows[0]
        } catch (error) {
